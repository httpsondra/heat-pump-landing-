...
(function () {
  ...
  /* Pojistka z <head> už není potřeba. Bez zrušení by se při pomalém CDN
     spustila i tak a reveal animace by pro celou návštěvu zůstaly mrtvé. */
  if (window.__revealFallbackTimer) {
    window.clearTimeout(window.__revealFallbackTimer);
    html.classList.remove('reveal-fallback');
  }
  ...
  var topnav = doc.getElementById('topnav');   /* shodné s aria-controls */
  if (menuToggle && topnav) {
    var setMenu = function (open) {
      menuToggle.setAttribute('aria-expanded', open ? 'true' : 'false');
      topnav.classList.toggle('is-open', open);
      /* Zamkne scroll pod otevřeným menu a schová fixní mobilní lištu —
         dvě navigační plochy naráz jsou matoucí a lišta by dropdown překryla. */
  ...
  /* =======================================================================
     Hero se spouští až ve chvíli, kdy je jasné, jestli GSAP dorazí.
     Knihovny se totiž nově načítají jen na desktopu a asynchronně, takže
     synchronní `typeof window.gsap` by na rychlém stroji vyšel náhodně —
     a hero by podle načasování jednou scrolloval a podruhé ne.
     ======================================================================= */
  function startHero() {
    /* =======================================================================
    ...
    if (canvas && !prefersReduced && gsapReady) {
      var modeAtLoad = smallScreen, modeRt, lastW = window.innerWidth;
      window.addEventListener('resize', function () {
        /* Jen změna šířky. Na Androidu i iOS vyvolá `resize` také otevření
           klávesnice a sbalení adresního řádku — a reload uprostřed
           vyplňování poptávky by byl to nejhorší, co může nastat.
           (Rozepsaná poptávka se navíc průběžně ukládá do sessionStorage
           ve form.js, takže i kdyby reload proběhl, odpovědi zůstanou.) */
    ...
    function enterFallbackMode() {
      ...
      /* Kdo si vyžádal omezený pohyb, nemá dostat automaticky přehrávané video —
         dřív dostával přesně to. Zůstane statický poster (frame 0). */
      if (prefersReduced) return;
      /* Data a baterie: na úsporném režimu nebo pomalé síti stačí poster. */
      var c = navigator.connection || {};
      if (c.saveData || /(^|-)2g$/.test(c.effectiveType || '')) return;
      if (!fallback) return;
      /* Video má 1,2 MB. Poster (51 kB) se ukáže okamžitě a je to LCP prvek,
         takže video spouštíme až po `load` — jinak soupeří o pásmo s fonty,
         CSS a posterem přesně ve chvíli, kdy se stránka vykresluje. */
      var startVideo = function () {
        fallback.setAttribute('preload', 'auto');
        // autoplay až po načtení; muted+playsinline je v markupu
        var tryPlay = function () {
          var pr = fallback.play();
          if (pr && typeof pr.catch === 'function') pr.catch(function () {/* uživatel spustí sám */});
        };
        if (fallback.readyState >= 2) tryPlay();
        else fallback.addEventListener('loadeddata', tryPlay, { once: true });
        fallback.load();
      };
      if (doc.readyState === 'complete') startVideo();
    ...
    function setupScrollTrigger() {
      if (ready || fellBack) return;   /* pozdní snímek už nesmí pinovat rozložené hero */
      ready = true;
    ...
    fetch('/images/sequence/manifest.json?v=30', { cache: 'force-cache' })
      .then(function (r) {
      ...
      .then(function (m) {
        ...
        window.setTimeout(function () {
          if (!firstDrawn && !fellBack && nearestLoaded(0)) {
            firstDrawn = true;
            if (loader) loader.classList.add('is-hidden');
            resizeCanvas();
            setupScrollTrigger();
  ...
  if (window.__gsapPending) {
    var heroStarted = false;
    var launch = function () { if (heroStarted) return; heroStarted = true; startHero(); };
    window.addEventListener('md:gsap', launch, { once: true });
    window.setTimeout(launch, 6000);   /* CDN mlčí → spustit ve video režimu */
  } else {
    startHero();
  }
})();
